...
from android.agentic_workflow import (
...
# Register Android-specific Gradle validation provider for run_validation_command tool
register_android_validation_provider()
...
def _make_wait_for_user_input(*, task_id: str, callback_url: str):
    ...
    def _wait(question: str) -> str | None:
        ...
        _notify_callback(
            ...
            f"Android Agent needs input: {question}",
        )
...
def _resolve_jira_context_from_metadata(
    ...
    """Extract ticket key and content from Team Lead-handed-off metadata."""
...
def _run_workflow(task_id: str, message: dict) -> None:
    """
    Agentic Android Agent workflow running in a background thread.
    ...
    runtime_config = build_android_agent_runtime_config()
    ...
    task_store.update_state(task_id, "TASK_STATE_WORKING", "Android Agent is starting.")
    audit_log("TASK_STARTED", task_id=task_id, compass_task_id=compass_task_id)

    task_prompt = build_android_task_prompt(
        user_text=user_text,
        workspace=workspace,
        compass_task_id=compass_task_id,
    ...
    try:
        result = runtime.run_agentic(
            ...
            tools=ANDROID_AGENT_RUNTIME_TOOL_NAMES,
            max_turns=80,
            timeout=TASK_TIMEOUT,
        )

        summary = result.summary or "Android Agent task completed."
        final_artifacts = [
            build_text_artifact(
                "android-agent-summary",
                summary,
                metadata={
                    "agentId": AGENT_ID,
        ...
        if result.success:
            ...
            _update_task_artifacts(task_id, final_artifacts)
        ...
        task_store.update_state(
            task_id, "TASK_STATE_FAILED", f"Android Agent failed: {error_text[:500]}"
        )
        _update_task_artifacts(task_id, failure_artifacts)
        audit_log("TASK_FAILED", task_id=task_id, error=error_text[:300])
        _notify_callback(
            callback_url,
            task_id,
            "TASK_STATE_FAILED",
            f"Android Agent failed: {error_text[:500]}",
            failure_artifacts,
        )
...
def _update_task_artifacts(task_id: str, artifacts: list) -> None:
    task = task_store.get(task_id)
    if task:
