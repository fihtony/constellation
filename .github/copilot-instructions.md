...
- **Boundary ownership rule**: Workspace directories owned by a boundary agent must be written only by that owning agent. Orchestrators must request the boundary capability and consume the returned artifact paths; they must not backfill or synthesize another agent's workspace outputs. Example: only Jira Agent may write `jira/<ticket-key>/...`; Team Lead may request `jira.ticket.fetch` and then pass the returned Jira folder/path downstream, but must not write Jira-owned files itself. Exception: a repository cloned under `scm/<repo>/` becomes the shared development working tree, so development agents may create branches, edit source files, install dependencies, build, test, and commit inside that cloned repo.
...
- In v2 local/container delivery, shared runtime credentials must be mounted only into agents that explicitly declare `runtime_env_required: true` in their agent config. Boundary adapters (`jira`, `scm`, `ui-design`) must not mount the shared runtime env file. Per-task children such as `office` inherit runtime settings only through `launch_spec.pass_through_env`; they must not parse the shared runtime env file directly inside the launcher path.
...
- Persistent v2 agents launched via `scripts/run_local.py` should serve with `ThreadingHTTPServer`, not plain `HTTPServer`, so `/health` and `/tasks` stay responsive while long `/resume` requests are running.
- Compass per-task Office launch may receive host absolute paths from local pytest or operator calls; translate those host paths through the current container mounts before bind planning so the child Office container still gets a valid host bind source.
...
- Team Lead development context gathering must fail closed when Jira returns an error, a non-success status, a missing ticket/key, or a key mismatch. Repo URL must come from the user request or validated Jira context; do not fall back to a default repository when Jira/repo context is missing.
...
- Repo-backed development agents must work inside the shared-workspace clone on a local development branch, run local build/test validation before PR creation, and persist branch/test/PR evidence in their agent workspace. For UI/frontend/design tasks, Team Lead must set `definition_of_done.screenshot_required=true`; Web Agent must capture PNG implementation screenshots, upload/embed them in the PR description through SCM capabilities (`scm.pr.image.upload` / `scm.pr.update`), and expose `screenshotIncluded` / `screenshotUploaded` in callback artifact metadata. HTML snapshots alone do not satisfy this gate.
...
- Parent-to-child permission handoff must be explicit and child-scoped. When dispatching a child agent, the parent must attach `message.metadata.permissions` that is derived for that child (for example from the child's execution contract / permission profile), and must not forward the parent's own runtime permission snapshot. Child agents must use only the permission snapshot explicitly received in `message.metadata.permissions`; if it is missing, they must fail closed and treat the child as having no boundary permissions. Do not fall back to the parent agent's runtime permissions, a locally generated snapshot, or any other agent's permission context. This rule is required for future signed permission-set enforcement where snapshots are bound to a specific agent identity.
