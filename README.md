...
AGENT_RUNTIME_IMAGE=constellation-agent-runtime:latest
DYNAMIC_AGENT_NETWORK=constellation-network
```
...
1. Tracker 回归只允许读取和修改 `TEST_JIRA_TICKET_URL` 中指定的 ticket，并且所有写操作都必须恢复原状态。
2. SCM 回归只允许读取和修改 `tests/agent_test_targets.json` 中定义的测试仓库，写入路径固定在 `agent-tests/` 下。
...
```text
https://your-org.atlassian.net/browse/PROJ-1
```

agent 会从这个显式 URL 解析 ticket key，再访问对应 API，例如：

```text
https://your-org.atlassian.net/rest/api/3/issue/PROJ-1
```

如果当前 token 是 scoped token，agent 仍会自动改走：

```text
https://api.atlassian.com/ex/jira/{cloudId}/rest/api/3/issue/{ticketKey}
```
...
curl -s -X POST http://localhost:8080/message:send \
  ...
  -d '{
    ...
    "message": {
      ...
      "role": "ROLE_USER",
      "parts": [{"text": "Please analyze https://your-org.atlassian.net/browse/PROJ-1"}]
    }
  }' | python3 -m json.tool
```
...
curl -s -X POST http://localhost:8080/message:send \
  ...
  -d '{
    "message": {
      ...
      "role": "ROLE_USER",
      "parts": [{"text": "Analyze https://your-org.atlassian.net/browse/PROJ-1 and prepare the implementation plan for https://github.com/your-username/test-repo."}]
    }
  }' | python3 -m json.tool
```
...
1. Tracker Ticket
   输入：`Please analyze https://your-org.atlassian.net/browse/PROJ-1`
...
3. Execution Workflow
  输入：`Analyze https://your-org.atlassian.net/browse/PROJ-1 and prepare the implementation plan for https://github.com/your-username/test-repo.`
